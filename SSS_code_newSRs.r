...
SSS.II<-function(filepath,file.name,reps=1000,seed.in=19,Dep.in=c(1,0.4,0.1),M.in=c(3,0.1,0.4,3,0.1,0.4),SR_type=3,h.in=c(1,0.6,0.2),FMSY_M.in=c(1,0.5,0.1),BMSY_B0.in=c(1,0.5,0.1),Winf=100,Amat=5,L1.in=c(0,0,0,0),Linf.in=c(0,0,0,0),k.in=c(0,0,0,0),Zfrac.Beta.in=c(-99,0.2,0.6,-99,0.5,2),R_start=6,sum_age=0,sb_ofl_yrs=c(2010,2011,2012),f_yr=2012,year0=1916,genders=F,doR0.loop=1)
{
  set.seed(seed.in)
  start.time<-Sys.time()
  Spp.quant.out<-list()
  ...
  if(SR_type>=8)
  {
    ...
    Input.draws.MQs<-as.data.frame(matrix(NA,nrow=reps,ncol=2))
    colnames(Input.draws.MQs)<-c("FMSY/M","BMSY/B0")
  }
  ...
  while(i<reps+1)
  {
    ...
    if(h.in[1]>=0 & length(h.in)==3 & FMSY_M.in[1]<0 & BMSY_B0.in[1]<0)
    {
      ...
      if(h.in[1]==30){h.draw<-round(rtlnorm(1,log(h.in[2]),h.in[3],0.21,0.99),2)}
    ...
    {
      SRdat.new<-readLines(paste(filepath,"/SR_MQs/BMSYB0.dat",sep=""))
      ...
      while(x==1)
      {
        ...
        if(FMSY_M.in[1]==30){FMSY_M.draw<-round(rtlnorm(1,log(FMSY_M.in[2]),FMSY_M.in[3],0.0001,10),2)}
        if(FMSY_M.in[1]==4){FMSY_M.draw<-round(runif(1,FMSY_M.in[2],FMSY_M.in[3]),2)}
        if(BMSY_B0.in[1]==2){BMSY_B0.draw<-round(rbeta.ab(1,BMSY_B0.in[2],BMSY_B0.in[3],0.05,0.95),2)*100}
        if(BMSY_B0.in[1]==10){BMSY_B0.draw<-round(rtnorm(1,BMSY_B0.in[2],BMSY_B0.in[3],0.05,0.95),2)*100}
        if(BMSY_B0.in[1]==30){BMSY_B0.draw<-round(rtlnorm(1,log(BMSY_B0.in[2]),BMSY_B0.in[3],0.05,0.95),2)*100}
        ...
        SRdat.new.mod[9]<-FMSY_M.draw * M.draw
        SRdat.new.mod[10]<-BMSY_B0.draw
        SRdat.new[grep("Amax",SRdat.new)+1][1]<-paste(SRdat.new.mod,collapse=" ")
        write(SRdat.new,paste(filepath,"/SR_MQs/BMSYB0.dat",sep=""))
        RUN.SS(paste(filepath,"/SR_MQs/",sep=""), ss.exe="BMSYB0",ss.cmd=" -nox > out.txt")
        SRMQ_rep.new<-readLines(paste(filepath,"/SR_MQs/bmsyb0.rep",sep=""))
        SRMQ_par[i]<-readLines(paste(filepath,"/SR_MQs/bmsyb0.par",sep=""))[[1]][1]
        steep_expo.out<-as.numeric(strsplit(SRMQ_rep.new[1]," ")[[1]])
        if(steep_expo.out[1]<0.21){x<-1}
        if(steep_expo.out[1]>0.99){x<-1}
        if(steep_expo.out[1]>=0.21&steep_expo.out[1]<=0.99){x<-2}
      }
    ...
    if(Zfrac.Beta.in[1]>=0)
    {
      ...
      if(Zfrac.Beta.in[1]==30){Zfrac.draw<-round(rtlnorm(1,log(Zfrac.Beta.in[2]),Zfrac.Beta.in[3],0,1),2)}
    ...
    if(Zfrac.Beta.in[4]>=0)
    {
      ...
      if(Zfrac.Beta.in[4]==30){Beta.draw<-round(rtlnorm(1,log(Zfrac.Beta.in[5]),Zfrac.Beta.in[6],0,10),2)}
    ...
    if(Dep.in[1]>=0)
    {
      ...
      if(abs(Quant.out[i,22]-Quant.out[i,23])<0.01&Quant.out[i,28]==0&Quant.out[i,14]<100000){i<-i+1}
      else
      {
        ...
        else
        {
          ...
          n.bad<-n.bad+1
          if(length(M.in)>6|length(Dep.in)>3|length(h.in)>3){i<-i+1}
        }
      }
    }
  ...
  if(SR_type>=8){Input.draws<-cbind(Input.draws,Input.draws.MQs,SRMQ_par)}
  end.time<-Sys.time()
